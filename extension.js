...
async function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("cloudsmith-vsc.inspectUpstreams", async (repoNode) => {
      if (!repoNode) {
        vscode.window.showWarningMessage("No repository selected.");
        return;
      }

      const workspace = repoNode.workspace;
      const repoSlug = repoNode.slug;
      const repoName = repoNode.name;
