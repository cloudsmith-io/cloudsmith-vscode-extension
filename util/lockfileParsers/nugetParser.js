...
const nugetParser = {
  ...
  async detect(workspaceFolder) {
    ...
    const lockfilePath = await pathExists(path.join(rootPath, "packages.lock.json"))
      ? path.join(rootPath, "packages.lock.json")
      : null;
    const manifestPath = csprojPath ? path.join(rootPath, csprojPath) : null;
  ...
  async resolve({ lockfilePath, manifestPath }) {
    ...
    const manifestDependencies = manifestPath && await pathExists(manifestPath)
      ? parseCsprojManifest(await readUtf8(manifestPath))
      : [];
