...
const rubyParser = {
  ...
  async canResolve(workspaceFolder) {
    ...
    return (await pathExists(path.join(rootPath, "Gemfile.lock")))
      || (await pathExists(path.join(rootPath, "Gemfile")));
  },

  async detect(workspaceFolder) {
    const rootPath = getWorkspacePath(workspaceFolder);
    const lockfilePath = await pathExists(path.join(rootPath, "Gemfile.lock"))
      ? path.join(rootPath, "Gemfile.lock")
      : null;
    const manifestPath = await pathExists(path.join(rootPath, "Gemfile"))
  ...
  async resolve({ lockfilePath, manifestPath }) {
    ...
    if (!lockfilePath) {
      const dependencies = parseGemfileManifest(await readUtf8(manifestPath)).map((dependency) => createDependency({
    ...
    const directNames = manifestPath && await pathExists(manifestPath)
      ? new Set(parseGemfileManifest(await readUtf8(manifestPath)).map((dependency) => dependency.name.toLowerCase()))
      : null;
    const records = parseGemfileLock(await readUtf8(lockfilePath));
