...
const cargoParser = {
  ...
  async canResolve(workspaceFolder) {
    ...
    return (await pathExists(path.join(rootPath, "Cargo.lock")))
      || (await pathExists(path.join(rootPath, "Cargo.toml")));
  },

  async detect(workspaceFolder) {
    const rootPath = getWorkspacePath(workspaceFolder);
    const lockfilePath = await pathExists(path.join(rootPath, "Cargo.lock"))
      ? path.join(rootPath, "Cargo.lock")
      : null;
    const manifestPath = await pathExists(path.join(rootPath, "Cargo.toml"))
  ...
  async resolve({ lockfilePath, manifestPath }) {
    const manifestDependencies = manifestPath && await pathExists(manifestPath)
      ? parseCargoTomlManifest(await readUtf8(manifestPath))
      : [];
    ...
    const records = parseCargoLock(await readUtf8(lockfilePath));
    if (records.length === 0) {
